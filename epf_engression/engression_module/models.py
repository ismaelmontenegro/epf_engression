...
class StoLayer(nn.Module):
    """A stochastic layer (optionally with state-dependent noise)."""
    def __init__(self, in_dim, out_dim, noise_dim=100, add_bn=False, out_act=None,
                 noise_std=1, verbose=True,
                 hetero_noise=False, cond_dim=None, scale_hidden_dim=64):
        super().__init__()
        self.in_dim = in_dim
        ...
        # ----- state-dependent noise scale: scale(x) = softplus(scale_head(cond)) -----
        self.hetero_noise = hetero_noise and noise_dim > 0
        if self.hetero_noise:
            ...
            self.scale_head = nn.Sequential(
                ...
                nn.ELU(),
                nn.Linear(scale_hidden_dim, noise_dim),
            )
            # init so initial scale == noise_std for any input (starts with fixed-scale):
            # softplus(b) = noise_std  ->  b = log(exp(noise_std) - 1)
            nn.init.zeros_(self.scale_head[-1].weight)
            nn.init.constant_(self.scale_head[-1].bias,
                              math.log(math.expm1(float(noise_std))))

    def _noise(self, batch_size, n_noise, device, cond):
        if self.hetero_noise and cond is not None and n_noise == self.noise_dim:
            scale = F.softplus(self.scale_head(cond))          # (batch, noise_dim), > 0
            return torch.randn(batch_size, self.noise_dim, device=device) * scale
        return torch.randn(batch_size, n_noise, device=device) * self.noise_std
    ...
    def forward(self, x, cond=None):
        ...
        if isinstance(x, int):
            ...
            out = self._noise(x, self.noise_dim, device, cond)
        else:
            ...
            n_noise = self.noise_dim + self.in_dim - x.size(1)
            eps = self._noise(x.size(0), n_noise, device, cond)
...
class StoNet(StoNetBase):
    """Stochastic neural network (optionally heteroscedastic noise)."""
    def __init__(self, in_dim, out_dim, num_layer=2, hidden_dim=100,
                 ...
                 hetero_noise=False, scale_cond_idx=None, scale_hidden_dim=64, hetero_output = False, output_scale_hidden_dim = 64):
        ...
        # columns of x that drive the noise scale (None -> condition on the full input)
        if hetero_noise and scale_cond_idx is not None:
            self.register_buffer("scale_cond_idx",
                                 torch.as_tensor(scale_cond_idx, dtype=torch.long))
            cond_dim = len(scale_cond_idx)
        else:
            self.scale_cond_idx = None
            cond_dim = in_dim
        ...
        if self.resblock:
            ...
            self.input_layer = StoLayer(in_dim=in_dim, out_dim=hidden_dim, noise_dim=noise_dim,
                                        add_bn=add_bn, out_act="elu", verbose=verbose,
                                        hetero_noise=hetero_noise, cond_dim=cond_dim,
                                        scale_hidden_dim=scale_hidden_dim)
            inter_noise_dim = noise_dim if noise_all_layer else 0
            self.inter_layer = nn.ModuleList([
                StoLayer(in_dim=hidden_dim, out_dim=hidden_dim, noise_dim=inter_noise_dim,
                         add_bn=add_bn, out_act="elu",
                         hetero_noise=hetero_noise, cond_dim=cond_dim,
                         scale_hidden_dim=scale_hidden_dim)
        ...
        self.hetero_output = hetero_output  # new flag
        if self.hetero_output:
            # output-space scale head: cond -> out_dim, one scale per subperiod
            self.output_scale_head = nn.Sequential(
                nn.Linear(cond_dim, output_scale_hidden_dim),
                nn.ELU(),
                nn.Linear(output_scale_hidden_dim, out_dim),
            )
            # init to identity: softplus(b) = 1  ->  b = log(e^1 - 1) = log(expm1(1))
            nn.init.zeros_(self.output_scale_head[-1].weight)
            nn.init.constant_(self.output_scale_head[-1].bias, math.log(math.expm1(1.0)))

    def _cond(self, x):
        if not self.hetero_noise:
            return None
        if self.scale_cond_idx is None:
            return x
        return x.index_select(1, self.scale_cond_idx)
    ...
    def forward(self, x):
        if self.num_blocks == 1:
            ...
            cond = self._cond(x)
            h = self.input_layer(x, cond)
            for layer in self.inter_layer:
                h = layer(h, cond)
            out = self.out_layer(h)

        if self.hetero_output:
            # use full x as conditioning, independent of hetero_noise flag
            out_cond = x if self.scale_cond_idx is None else x.index_select(1, self.scale_cond_idx)
            s = F.softplus(self.output_scale_head(out_cond))
            out = out * s
